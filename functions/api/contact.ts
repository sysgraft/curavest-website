 ...
 *
 * Handles submissions from the "Start a Conversation" form. This endpoint
 * is intentionally isolated from the rest of the (fully static) site so it
 * can be wired up to a real email provider without touching the Astro build.
 *
 * REQUIRED CONFIGURATION (not present in this environment — see README.md):
 *   - RESEND_API_KEY   Cloudflare Pages environment variable / secret.
 *                       Create a free account at https://resend.com, verify
 *                       the curavest.co.uk sending domain, and generate an
 ...
 *
 * Until RESEND_API_KEY is configured, this function responds with 503 and a
 * clear, honest message — the contact form's UI will never silently claim
 * success it can't back up. No credentials are hard-coded or invented here.
 */
